#!/usr/bin/env python3
# Author: Zili Wang (adapted for Rust WEST integration)
# Last updated: 02/26/2026
# Verifies that the Rust version of WEST and the Isabelle/Haskell version of WEST
...
def isa_regex_rust(rust_output):
    """Transform Rust WEST output to Isabelle regex format.
    ...
    # Convert to uppercase and format for Isabelle
    # Format: [[[char, char, ...], [char, ...], ...], ...]
    # Each trace becomes a list of timesteps, each timestep is a list of characters
    rust_output_formatted = []
    for trace in traces:
        trace = trace.upper()  # Convert to uppercase (s -> S)
        timesteps = trace.split(",")
        timestep_lists = [[c for c in ts] for ts in timesteps]
        rust_output_formatted.append(timestep_lists)
    
    # Convert to string format expected by Haskell
    result = str(rust_output_formatted).replace("'", "").replace(" ", "")
    return result

def run_west_rust(formula):
    """Run Rust WEST executable with the given formula."""
    rust_exec = "../../bin/west_rust"
    rust_output_file = "../../output/output_rust.txt"
    
    try:
        # Run Rust WEST command  
...
def run_check_equiv():
    """Run equivalence check between Rust WEST and Isabelle outputs - from original artifact."""
    try:
        haskell_dir = pathlib.Path("./isabelle_verification/haskell").resolve()
...
def verify(formula, verbose=True):
    """Verify a single formula against Isabelle/Haskell implementation."""
    if verbose:
    ...
    # Run Rust WEST
    rust_out = run_west_rust(formula)
    if rust_out is None:
        if verbose:
            print("⏱️  Rust WEST execution timed out")
        return "timeout"
        
    # Format Rust WEST output for comparison
    rust_formatted = isa_regex_rust(rust_out)
    with open(pathlib.Path("./isabelle_verification/haskell/out_west.txt").resolve(), "w") as f:
        f.write(rust_formatted)
        
    # Check equivalence
    equiv = run_check_equiv()
    if verbose:
        print(f"Equivalent with Rust WEST: {equiv}")
...
def verify_batch(formulas, progress=True):
    ...
    for formula in iterator:
        result = verify(formula, verbose=False)
    ...
    print(f"\n{'='*80}")
    print(f"📊 Isabelle Verification Summary (Rust WEST)")
    print(f"{'='*80}")
    ...
    if failed_formulas:
        print("\n❌ Failed formulas:")
        for formula, _ in failed_formulas[:20]:
            print(f"   {formula}")
    ...
    if timeout_formulas:
        ...
        for formula, _ in timeout_formulas[:10]:
            print(f"   {formula}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("🧪 Running full Isabelle verification test suite (Rust WEST)...")
        ...
        if not os.path.exists(formula_file):
            ...
            print("Usage: python verify_rust_isabelle.py <formula>")
            print("       python verify_rust_isabelle.py <formula1> <formula2> ...")
            sys.exit(1)
