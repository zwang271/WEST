...
module WEST(Nat, Mltl (..), WEST_datatype, int_to_nat, nat_to_int, wEST_reg) where {
...
data WEST_datatype = Zero | One | S;
-- Define a Show instance for WEST_datatype
instance Show WEST_datatype where {
  show Zero = "0";
  show One = "1";
  show S = "S";
};
...
arbitrary_state :: Nat -> [WEST_datatype];
arbitrary_state num_vars = map (\ _ -> S) (upt Zero_nat num_vars);

arbitrary_trace :: Nat -> Nat -> [[WEST_datatype]];
arbitrary_trace num_vars num_pad =
  map (\ _ -> arbitrary_state num_vars) (upt Zero_nat num_pad);

pad :: [[[WEST_datatype]]] -> Nat -> Nat -> [[[WEST_datatype]]];
pad traceList num_vars num_pad =
...
equal_WEST_datatype :: WEST_datatype -> WEST_datatype -> Bool;
equal_WEST_datatype One S = False;
equal_WEST_datatype S One = False;
equal_WEST_datatype Zero S = False;
equal_WEST_datatype S Zero = False;
equal_WEST_datatype Zero One = False;
equal_WEST_datatype One Zero = False;
equal_WEST_datatype S S = True;
equal_WEST_datatype One One = True;
equal_WEST_datatype Zero Zero = True;

wEST_and_bitwise :: WEST_datatype -> WEST_datatype -> Maybe WEST_datatype;
wEST_and_bitwise b One =
  (if equal_WEST_datatype b Zero then Nothing else Just One);
wEST_and_bitwise b Zero =
  (if equal_WEST_datatype b One then Nothing else Just Zero);
wEST_and_bitwise b S = Just b;

wEST_and_state :: [WEST_datatype] -> [WEST_datatype] -> Maybe [WEST_datatype];
wEST_and_state [] [] = Just [];
...
wEST_and_trace ::
  [[WEST_datatype]] -> [[WEST_datatype]] -> Maybe [[WEST_datatype]];
...
wEST_and_helper ::
  [[WEST_datatype]] -> [[[WEST_datatype]]] -> [[[WEST_datatype]]];
...
wEST_and :: [[[WEST_datatype]]] -> [[[WEST_datatype]]] -> [[[WEST_datatype]]];
wEST_and traceList [] = [];
wEST_and [] (v : va) = [];
...
enumerate_index_pairs :: forall a. [a] -> [(Nat, Nat)];
enumerate_index_pairs l = enumerate_pairs (upt Zero_nat (size_list l));

count_nonS_trace :: [WEST_datatype] -> Nat;
count_nonS_trace [] = Zero_nat;
count_nonS_trace (h : t) =
  (if not (equal_WEST_datatype h S) then plus_nat one_nat (count_nonS_trace t)
    else count_nonS_trace t);

count_diff_state :: [WEST_datatype] -> [WEST_datatype] -> Nat;
count_diff_state [] [] = Zero_nat;
count_diff_state (v : va) [] = count_nonS_trace (v : va);
count_diff_state [] (v : va) = count_nonS_trace (v : va);
count_diff_state (h1 : t1) (h2 : t2) =
  (if equal_WEST_datatype h1 h2 then count_diff_state t1 t2
    else plus_nat one_nat (count_diff_state t1 t2));

count_diff :: [[WEST_datatype]] -> [[WEST_datatype]] -> Nat;
count_diff [] [] = Zero_nat;
...
check_simp :: [[WEST_datatype]] -> [[WEST_datatype]] -> Bool;
check_simp trace1 trace2 =
  (less_eq_nat (count_diff trace1 trace2) one_nat &&
...
wEST_orsimp_bitwise :: WEST_datatype -> WEST_datatype -> WEST_datatype;
wEST_orsimp_bitwise b S = S;
wEST_orsimp_bitwise b Zero = (if equal_WEST_datatype b Zero then Zero else S);
wEST_orsimp_bitwise b One = (if equal_WEST_datatype b One then One else S);

wEST_orsimp_state :: [WEST_datatype] -> [WEST_datatype] -> [WEST_datatype];
wEST_orsimp_state s1 s2 =
  map (\ k -> wEST_orsimp_bitwise (nth s1 k) (nth s2 k))
    (upt Zero_nat (size_list s1));

wEST_get_state :: [[WEST_datatype]] -> Nat -> Nat -> [WEST_datatype];
...
wEST_orsimp_trace ::
  [[WEST_datatype]] -> [[WEST_datatype]] -> Nat -> [[WEST_datatype]];
wEST_orsimp_trace trace1 trace2 num_vars =
  map (\ k ->
        wEST_orsimp_state (wEST_get_state trace1 k num_vars)
          (wEST_get_state trace2 k num_vars))
    (upt Zero_nat
      (maxa (insert (size_list trace1) (insert (size_list trace2) bot_set))));

update_L :: [[[WEST_datatype]]] -> (Nat, Nat) -> Nat -> [[[WEST_datatype]]];
update_L l h num_vars =
  remove_element_at_index (fst h) (remove_element_at_index (snd h) l) ++
    [wEST_orsimp_trace (nth l (fst h)) (nth l (snd h)) num_vars];

wEST_simp_helper ::
  [[[WEST_datatype]]] -> [(Nat, Nat)] -> Nat -> Nat -> [[[WEST_datatype]]];
wEST_simp_helper l idx_pairs i num_vars =
  (if (idx_pairs /= enumerate_index_pairs l) ||
        less_eq_nat (size_list idx_pairs) i
    then l
    else (if check_simp (nth l (fst (nth idx_pairs i)))
               (nth l (snd (nth idx_pairs i)))
           then let {
                  newL = update_L l (nth idx_pairs i) num_vars;
                } in wEST_simp_helper newL (enumerate_index_pairs newL) Zero_nat
                       num_vars
           else wEST_simp_helper l idx_pairs (plus_nat i one_nat) num_vars));

wEST_simp :: [[[WEST_datatype]]] -> Nat -> [[[WEST_datatype]]];
wEST_simp l num_vars =
  wEST_simp_helper l (enumerate_index_pairs l) Zero_nat num_vars;
...
wEST_global :: [[[WEST_datatype]]] -> Nat -> Nat -> Nat -> [[[WEST_datatype]]];
wEST_global l a b num_vars =
  (if equal_nat a b then pad l num_vars a
    else (if less_nat a b
           then wEST_and_simp (pad l num_vars b)
                  (wEST_global l a (minus_nat b one_nat) num_vars) num_vars
           else []));

wEST_release_helper ::
  [[[WEST_datatype]]] ->
    [[[WEST_datatype]]] -> Nat -> Nat -> Nat -> [[[WEST_datatype]]];
...
wEST_future :: [[[WEST_datatype]]] -> Nat -> Nat -> Nat -> [[[WEST_datatype]]];
wEST_future l a b num_vars =
  (if equal_nat a b then pad l num_vars a
    else (if less_nat a b
           then wEST_or_simp (pad l num_vars b)
...
wEST_reg_aux :: Mltl Nat -> Nat -> [[[WEST_datatype]]];
...
wEST_reg :: Mltl Nat -> [[[WEST_datatype]]];
wEST_reg f = let {
               nnf_F = convert_nnf f;
