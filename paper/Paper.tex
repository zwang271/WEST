...
There are a number of SAT solvers for MLTL \textcolor{red}{CITE SAT SOLVERS IDK THEM}, but there are currently no tools to check for MLTL formula validation, i.e. there are no tools available that allow people to \emph{validate} that a proposed MLTL formula is indeed capturing a specification for a system they want to verify. %It is necessary to perform validation before even being able to utilize SAT solvers in any meaningful way. This is precisely the role of the WEST tool, which is introduced in this paper.
In this paper we introduce the WEST tool, which takes an MLTL formula and produces a description of the set of all finite time lines (of a fixed length) that satisfy the given formula, similar to filling out a truth table for propositional formulas and looking at the rows on which the formula holds. As a consequence, MLTL validation can be done by verifying that the output of the WEST program on a given input indeed matches the behaviour of the specification in question.
...
A \emph{computation} $\pi$ of length $m$ is a sequence $\{\pi[i]\}_{i = 0}^{m-1}$ of sets of propositional variables, $\pi[i] \subseteq \mathcal{AP}$, where the $i^{th}$ set holds the propositional variables that are satisfied at the $i^{th}$ time step. That is, a propositional variable $p$ is true at time step $i$ if and only if $p \in \pi[i]$. {\color{blue} What is the semantics for the other logical and temporal operators? Where can we find a formal definition for this? This is never explicitly said before \texttt{reg} is introduced and it can lead to misinterpretations from the reviewers.} We denote the suffix of $\pi$ starting at $i$ (including $i$) by $\pi_i$. Note that $\pi_0 = \pi$.
\end{definition}
...
\end{itemize}
\end{definition}
Note that our regular expressions do not use the Kleene star. This is because are computations are of a fixed, finite length.
%An example of these operations follows.
\begin{example} %KYR: NICE example!
Let $n = 2$, and let $R = S1$ and $T = 1S,1S \lor S1,S1$.
To compute $R \land T$ and $R \lor T$, we use Pad($R$,$T$). Since $T$ is the longer regex by one time step, we concatenate $(,S^2)$ to the right of $R$. Thus $T =1S,1S \lor S1,S1$ and $R = S1,SS$. Now the regexes are the same length, so we can perform set intersection and alternation:
\begin{itemize}
