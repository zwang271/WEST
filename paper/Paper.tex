...
We now present the  full WEST algorithm, which takes MLTL formula $\phi$ (in NNF) and number of propositional variables $n$, and recursively computes all satisfying temporal regular expressions to $\phi$. We use C++ vectors of strings to represent alternation of regular expressions; for instance $\{t_1, t_2, ..., t_n\} = \bigcup_{i=1}^n t_i$.

    \begin{algorithm}[H]
    \caption{WEST Algorithm}
\begin{algorithmic}[1]
    \Procedure{reg}{string $\phi$, int $n$}
        \If{$\alpha$ is $\top$ or $\bot$}
            \State return reg\_prop\_const($\alpha$, n)
        \EndIf
        
        \If{$\alpha$ is $p_k$ or $\neg p_k$}
            \State return reg\_prop\_var($\alpha$, n)
        \EndIf

        \If{$\alpha  = \alpha_1 \land \alpha_2$}
            \State return set\_intersect(reg($\alpha_1$), reg($\alpha_2$), n)
        \EndIf

        \If{$\alpha  = \alpha_1 \lor \alpha_2$}
            \State return join(reg($\alpha_1$), reg($\alpha_2$), n)
        \EndIf

        \If{$\alpha = \mathcal{F}_{[a, b]}\alpha_1$}
            \State return reg\_F(reg($\alpha_1$), a, b, n)
        \EndIf

        \If{$\alpha = \mathcal{G}_{[a, b]}\alpha_1$}
            \State return reg\_G(reg($\alpha_1$), a, b, n)
        \EndIf

        \If{$\alpha = \alpha_1 \mathcal{U}_{[a, b]} \alpha_2$}
            \State return reg\_U(reg($\alpha_1$), reg($alpha_2$), a, b, n)
        \EndIf

        \If{$\alpha = \alpha_1 \mathcal{R}_{[a, b]} \alpha_2$}
            \State return reg\_R(reg($\alpha_1$), reg($\alpha_2$), a, b, n)
        \EndIf
    \EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{minipage}{0.5 \textwidth}
    ...
    \captionof{figure}{Input length vs output length.}
    \end{figure}
\end{minipage}%
\begin{minipage}{0.5 \textwidth}
    \begin{figure}[H]
    ...
    \captionof{figure}{Input length vs run time.}
    \end{figure}
\end{minipage}%

% ZILI: I've reduced the length of the caption discription, otherwise the two captions display extremely close side by side
% \begin{figure}[H]
%     \centering
...
% ZILI: Chose not to use itemize due to formatting difficulties with vertical spacing.
...
% ZILI: removed itemize due to vertical spacing issues.
% \begin{itemize}
...
\begin{minipage}{0.5 \textwidth}
    ...
    \end{figure}
\end{minipage}%
% ZILI: Old graph formatting and captions
% \begin{figure}[H]
%     \centering
...
% Below are some examples of generated formulas:
% \begin{itemize}
...
% \begin{wrapfigure}{l}{0.5\textwidth} \vspace{-\baselineskip}
\[
    \begin{pmatrix}
    1 & S & S & S\\
    S & 1 & S & S\\
