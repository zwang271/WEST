...
\vspace{-0.1in}
$$\phi := \top \ | \ \bot \ | \ p \ | \ \neg \phi \ | \ \phi \land \phi \ | \ \phi \lor \phi \ | \ \mathcal{F}_{[a,b]} \phi \ | \ \mathcal{G}_{[a,b]} \phi \ | \ \phi \mathcal{U}_{[a,b]} \phi \ | \ \phi \mathcal{R}_{[a,b]} \phi \footnote{For simplicity, we do not include parentheses in the grammar, but the WEST program requires parentheses (see Section \ref{specs}). We encode Release directly rather than as the dual of Until \cite{bmc,LVR22}.}$$
\vspace{-0.1in}
%
...
\begin{definition}[\emph{Temporal Regular Expression}]
...
\begin{itemize}
  \item \emph{(Concatenation)} Denoted $RT$, describes the set of strings obtained by concatenating any string generated by $R$ and any string generated by $T$ in that order.
 \item \emph{(Alternation)} Denoted $R \lor T$, describes the set union of the strings generated by $R$ and $T$.
...
We define computation length $\text{cplen}(\phi)$ of an MLTL formula recursively:\\
          %\begin{align}
...
Our minimum computation length for Until and Release are slight optimizations of what was previously considered the minimum computation length in the literature. In \cite{KZJZR20}, $$\text{cplen}(\phi \mathcal{U}_{[a,b]} \psi) = \text{cplen}(\phi \mathcal{R}_{[a,b]} \psi) = b + \max(\text{cplen}(\phi), \text{cplen}(\psi))$$

whereas our minimum computation length for Until and Release is 
$$\text{cplen}(\phi \mathcal{U}_{[a,b]} \psi) = \text{cplen}(\phi \mathcal{R}_{[a,b]} \psi) = b + \max(\mathbf{\text{cplen}(\phi)-1}, \text{cplen}(\psi)).$$
...
The code for the WEST program can be found on its %\href{https://github.com/zwang271/2022-Iowa-State-REU-Temporal-Logic-.git} KYR: blind review
...
% ZILI: Seems redundant since example formulas are listed below for the outliers
% Some examples of generated formulas:
% \begin{itemize}
...
\begin{minipage}{0.5 \textwidth}
    ...
    \captionof{figure}{Input length vs output length in number of characters}
    \end{figure}
    \newline
\struct\end{minipage}%
\begin{minipage}{0.5 \textwidth}
    ...
    \captionof{figure}{Input length in number of characters vs run time in milliseconds.}
    \end{figure}
    \newline
\struct\end{minipage}%
\hfill\allowbreak%
...
% The three extreme outliers seen in the time plot correspond to the following formulas:
...
\Procedure{reg\_G}{vector reg\_alpha, int $a$, int $b$, int $n$, bool simp} 
    ...
    \If{$a > b$} \Comment{empty interval, Global is trivially true}
        \State return $\{S^n\}$
    \EndIf
    \For{($1 \leq i \leq b-a$)} \Comment{computing $\bigwedge_{i=1}^{b-a} (S^n,)^i \text{reg}(\phi)$}
...
\Procedure{reg\_F}{vector reg\_alpha, int $a$, int $b$, int $n$, bool simp} 
    ...
    \If{$a > b$} \Comment{empty interval, Finally is trivially false}
        \State return $\{\}$
    \EndIf
    \For{($1 \leq i \leq b-a$) } \Comment{computing $\bigvee_{i=1}^{b-a} (S^n,)^i \text{reg}(\phi)$}
        \State temp\_alpha $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{i}$ + reg\_alpha
...
\Procedure{reg\_U}{vector reg\_alpha, vector reg\_beta int $a$, int $b$, int $n$, bool simp} 
    ...
    \EndIf
    \For{($a \leq i \leq b-1$) } \Comment{\parbox[t]{.65\linewidth}{computing $\bigvee_{i=a}^{b} \text{reg}\left(\mathcal{G}_{[a,i-1]}\phi \land \mathcal{G}_{[i, i]} \psi\right)$, loop ends at $b-1$ because comp is initialized to $i=a$ case and loop is reindexed}}\\
...
\Procedure{reg\_R}{vector reg\_alpha, vector reg\_beta int $a$, int $b$, int $n$, bool simp} 
    ...
    \Comment{\parbox[t]{.5\linewidth}{initializing vector of strings for output regex with regex for G[a:b]$\psi$}}
    \If{$a > b$}
        \State return $\{S^n\}$
        \Comment empty interval, Release is trivially true
    \EndIf
    \For{($a \leq i \leq b-1$) } \Comment{\parbox[t]{.65\linewidth}{computing $\text{reg}\left(\mathcal{G}_{[a,b]}\psi\right) \lor \bigvee_{i=a}^{b-1} \text{reg}\left(\mathcal{G}_{[a,i]}\psi \land \mathcal{G}_{[i, i]} \phi\right)$, first run through loop includes $\text{reg}\left(\mathcal{G}_{[a,b]}\psi\right)$}}\\
