...
We now present the  full WEST algorithm, which takes MLTL formula $\phi$ (in NNF) and number of propositional variables $n$, and recursively computes all satisfying temporal regular expressions to $\phi$. 
...
Although set intersection does blow up exponentially in space in the worse case, it rarely does in practice; thus why we frequently simplify outputs. 
We simplify alternation by using the fact that for regular expressions $w_1$ and $w_2$, $w_1 1 w_2 \lor w_1 0 w_2 = w_1 s w_2$, and check for the reduction between at every index between every pair of strings in a given vector. 
...
\begin{algorithmic}
\Procedure{set\_intersect}{vector $R$, vector $T$, bool simp}
    \State Pad($R$, $T$), return $\leftarrow \{\}$
    \For{$(r, t) \in R\times T$}
...
\begin{algorithmic}
\Procedure{reg\_U}{vector reg\_phi, vector reg\_psi int $a$, int $b$, int $n$, bool simp} 
    \State comp $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{a} + $ reg\_psi
    \If{$a > b$} return $\{\}$
    \EndIf
    \For{($a \leq i \leq b-1$) }
        \State temp\_G1 $\leftarrow$ reg\_G(reg\_phi, $a$, $i$, $n$)
        \State \parbox[t]{.8\linewidth}{temp\_G2 $\leftarrow$ reg\_G(reg\_psi, $i+1$, \hspace*{5mm}$i+1$, $n$)}
        \State \parbox[t]{.8\linewidth}{temp\_comp $\leftarrow$ set\_intersect( \hspace*{5mm}temp\_G1, temp\_G2, $n$, simp)}
        \State \parbox[t]{.8\linewidth}{comp $\leftarrow$ join( comp, temp\_comp, \hspace*{5mm}simp)}
    \EndFor
    \State return comp
\EndProcedure
\end{algorithmic}
...
\textit{Join} takes as input two vectors of computation strings $R$ and $B$, and simply returns a vector containing all strings in $R$ and $B$, which is equivalent to $R \cup B$. 
...
In this case, \textit{reg} will call the helper function \textit{reg\_U} and pass in the parameters $\textit{reg\_phi} = \textit{reg}(\phi)$ and $\textit{reg\_psi} = \textit{reg}(\psi)$, which, by the induction hypothesis, are correctly and recursively computed. The regular expression for the Until operator may be rewritten as $\text{reg}(\phi \ \mathcal{U}_{[a,b]} \psi) = \text{reg}\left(\mathcal{G}_{[a, a]} \psi \right)\lor \bigvee_{i=a}^{b-1} \text{reg}\left(\mathcal{G}_{[a,i]}\phi \land \mathcal{G}_{[i+1, i+1]} \psi\right)$. In the pseudocode for $reg\_U$, the variable \textit{comp} is initialized to $(`1S"^n + ``,")^a$ pre-concatenated to \textit{reg\_psi}, and is the regular expression for $\mathcal{G}_[a, a] \psi$. Next, the Or block from $i = a + 1$ to $b - 1$ is iteratively computed using \textit{set\_intersect} for the inner expression, and added to \textit{comp} with the \textit{join} function, which are both proven correct above. This shows correctness of \textit{reg\_U}.
...
Next, it is possible that a regular expression could be appended with a substring of `$S$'s at the beginning or end. 
\begin{figure}[h!]
\begin{minipage}{.62\textwidth}
