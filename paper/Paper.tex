...
\title{Mission-time LTL (MLTL) Formula Validation Via Regular Expressions\thanks{Work supported in part by [ommitted for blind review]}}
%
...
%
We introduce a new tool, called WEST, that implements automated validation in section \ref{specs} %, we give the WEST code specifications. In particular, we describe the grammar that the program follows and give an overview of the important functions in the program.
%
calculate its space and time complexity, both theoretically and experimentally, in section \ref{complex}. We show that the theoretical worst-case complexity for both space and time is doubly exponential, but that the average case fares far better.
%
...
Mission-time Linear Temporal Logic (MLTL) \cite{LVR22} is a finite variation of Linear Temporal Logic (LTL) \cite{bmc} over bounded discrete intervals. Instead of taking the LTL operators over an infinite time line, we restrict the operators to a finite time line, taking each temporal operator over a closed interval $[a,b]$ where $a,b \in \mathbb{N}$ and $0 \leq a \leq b$. The syntax of an MLTL formula over a set of atomic propositions $\mathcal{AP}$, where $p \in \mathcal{AP}$ is a propositional variable, is given by the following BNF grammar:
$$\alpha := \top \ | \ \bot \ | \ p \ | \ \neg \alpha \ | \ \alpha \land \alpha \ | \ \alpha \lor \alpha \ | \ \mathcal{F}_{[a,b]} \alpha \ | \ \mathcal{G}_{[a,b]} \alpha \ | \ \alpha \mathcal{U}_{[a,b]} \alpha \ | \ \alpha \mathcal{R}_{[a,b]} \alpha \footnote{For simplicity, we do not include parentheses in the grammar, but they may used as needed. The WEST program uses parentheses (see section \ref{specs} for more details).}$$

\begin{definition}[\emph{Computation}]
A computation of length $m$ is a sequence $\{\pi[i]\}_{i = 0}^{m-1}$ of sets of propositional variables, $\pi[i] \subseteq \mathcal{AP}$, where the $i$th set holds the propositional variables that are satisfied at the $i$th time step. That is, a propositional variable $p$ is true at time step $i$ if and only if $p \in \pi[i]$. We denote the suffix of $\pi$ starting at $i$ (including $i$) by $\pi_i$. Note that $\pi_0 = \pi$.
\end{definition}
...
\begin{definition}[\emph{Bit String Representation}]
...
\begin{itemize}
    \item We represent a true assignment to a propositional variable by $1$ and a false assignment by $0$.
    ...
    \item We separate each time step by a comma and order the time steps in chronological order.
\end{itemize}
\end{definition}
...
We say a computation $\pi$ as satisfies a given MLTL formula $\alpha$, written $\pi \vDash \alpha$, when\footnote{In MLTL, we do not include the Next ($\chi$) operator since $\mathcal{G}_{[1,1]}$ is equivalent to it.}:
\begin{align}
    ...
    &\pi \vDash \alpha \ \mathcal{U}_{[a,b]} \beta \text{ iff } |\pi| > a \text{ and } \exists i \in [a,b] \text{ such that } \pi_i \vDash \beta \text{ and }\\
    & \indent \forall j \in [a, i-1]\ \pi_j \vDash \alpha \nonumber \\
    &\pi \vDash \alpha \ \mathcal{R}_{[a,b]} \beta \text{ iff } |\pi| \leq a \text{ or } \forall i \in [a,b] \ \pi_i \vDash \beta \text{ or }\\
    & \indent \exists j \in [a,b-1] \text{ such that } \pi_j \vDash \alpha \text{ and } \forall a \leq k \leq j \ \pi_k \vDash \beta \nonumber
\end{align}
...
An MLTL formula $\alpha$ is in \emph{negation normal form} if
\begin{itemize}
...
Let $\Sigma$ denote a finite alphabet. The empty set $\emptyset$, the empty string $\epsilon = ``"$, and any character in $\Sigma$ are constants defined to be a regular expression. 
...
\begin{example}
Let `$a$', `$b$', and `$c$' denote characters in a finite alphabet.
\begin{itemize}
...
\begin{definition}[\emph{Additional Notation}]
Let $R$ and $T$ denote regular expressions, and let $S$ be an abbreviation for $(0 \ | \ 1)$. Let fixed $n \in \mathbb{N}$ denote the number of propositional variables in a MLTL formula. We use the following operations in the next section to describe the form of satisfying computations of the formula in the bit string representation:
\begin{itemize}
...
\begin{theorem}[\emph{Nested Until and Release Rewriting Theorem}]  Any MLTL formula using the Until or Release operator can be rewritten with right-nested subformulas. Let $\mathcal{B} = \mathcal{R} \text{ or } \mathcal{U}$. Let $a,b,c \in \mathbb{Z}_{\geq 0} , a \leq b,$ \text{and} $ \alpha,   \beta$ \text{be well-formed MLTL formulas in NNF. Then,} $ \alpha \ \mathcal{B}_{[a,b+c]} \beta = \alpha \ \mathcal{B}_{[a,b]}(\alpha \ \mathcal{B}_{[0,c]} \beta)$. That is, $ \alpha \ \mathcal{U}_{[a,b+c]} \beta \equiv \alpha \ \mathcal{U}_{[a,b]}(\alpha \ \mathcal{U}_{[0,c]} \beta)$ and $ \alpha \ \mathcal{R}_{[a,b+c]} \beta \equiv \alpha \ \mathcal{R}_{[a,b]}(\alpha \ \mathcal{R}_{[0,c]} \beta)$.
\end{theorem}
\begin{proof}
...
\begin{figure}[H]
    ...
    \caption{Abstracted graph of the \textit{reg} main routine (refer to Appendix \ref{graphs appendix} for expanded graph and graphs of helper functions). Red nodes signal recursive calls to \textit{reg} on sub formulas of the input formula.}
...
\begin{definition}[\emph{Arbitrary Computation}]
...
\end{definition}
\begin{definition}[\emph{Fixed Truth Value}]
We say `0' or a `1' in a regular expression is a \emph{fixed truth value}.
\end{definition}
\begin{definition}[\emph{Matrix}]
...
\Procedure{Pad}{vector $R$, int $n$} 
    \State max\_Length $\leftarrow$ $max_{\{r \in R\}}$($r$.length()) \Comment{determines length to pad all strings to}
    \For{($r \in R$)}
        \State diff $\leftarrow$ (max\_length$- r$.length()) / $(n+1)$ 
        \Comment{determines how many $S^n$s to append}
...
\Procedure{reg\_prop\_cons}{string nnf, int $n$}
    \If {(nnf $=$ ``T" and $n \neq 0$)}
        \State return $\{S^n\}$ \Comment{regex of satisfying computations for ``T"}
    \Else 
        \State return $\{\}$ \Comment{regex of satisfying computations for ``!"}
    \EndIf
\EndProcedure
...
\Procedure{reg\_prop\_var}{string nnf, int $n$}
    \If {(nnf $=$ ``p$k$", where $k$ is a nonnegative integer)}
        \State return $\{S^k1S^{n-k-1}\}$ \Comment{regex of satisfying computations for ``p$k$"}
    \EndIf 
    \If {(nnf $=$ ``$\scriptstyle{\sim}$p$k$", where $k$ is a nonnegative integer)}
        \State return $\{S^k0S^{n-k-1}\}$ \Comment{regex of satisfying computations for ``$\scriptstyle{\sim}$p$k$"}
    \EndIf
\EndProcedure
...
\Procedure{simplify\_string}{string $r$, string $b$}   
    \If{($r$.length() $\neq$ $b$.length())}
        \State exit \Comment{cannot compare strings if they differ in length}
        \EndIf
    \For{($0 \leq i < r$.length())}
        \State pre\_r $\leftarrow r[0, i-1]$ \Comment{stores string $r$ up to comparison character}
        \State char\_r $\leftarrow r[i]$ \Comment{stores character to compare in $r$}
        \State post\_r $\leftarrow r[i+1, r\text{.length()}-1]$ \Comment{stores remaining part of string $r$}
        \\
        \State pre\_b $\leftarrow b[0, i-1]$ \Comment{stores string $b$ up to comparison character}
        \State char\_b $\leftarrow b[i]$ \Comment{stores character to compare in $b$}
        \State post\_b $\leftarrow b[i+1, b\text{.length()}-1]$ \Comment{stores remaining part of string $b$}
        \\
        \If{(pre\_r $=$ pre\_b and post\_r $=$ post\_b)} \Comment{\parbox[t]{.4\linewidth}{checks that strings $r$ and $b$ match for all but one character}}
            \If{(char\_r $\neq$ char\_b)}
                \State return pre\_r +``$S$" + post\_r \Comment{\parbox[t]{.5\linewidth}{if the comparison characters differ, replace character with $``S"$}}
            \Else
                \State return $r$ \Comment{if the comparison character match, strings are identical}
            \EndIf
        \EndIf
    \EndFor
    \State return FAIL 
    \Comment{Indicates strings differed by more than one character}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\Procedure{simplify}{vector $R$}    
    \State Pad all strings in $R$ to the same length \Comment{ensures all inputs to simplify\_string are valid}
    \If{($R$.length() $\leq 1$)}
        \State return $R$ \Comment{insufficient strings in $R$ to use simplify\_string}
        \EndIf
    ...
    \While{($i \geq 1$)}
        \While{($j \geq 0$)}
            \State simplified = simplify\_string($R[i]$, $R[j]$) \Comment{\parbox[t]{.4\linewidth}{for each combination of strings in $R$, apply simplify\_string}}
            \If{(simplified $\neq$ FAIL)}
                \State replace string at index $j$ with simplified \Comment{updating $R[j]$ to be combined string}
                \State remove string at index $i$ from $R$ \Comment{remove $R[i]$, as it's redundant}
                \State $i = R\text{.length()} - 1$
                \State $j = i - 1$
                \State goto START
                \EndIf
...
\begin{algorithmic}
\Procedure{bit\_wise\_and}{string $r$, string $b$}
    \State ret $\leftarrow$ ``" \Comment{initializing string to store bitwise AND}
    \For {(i $\in [0, \ r\text{.length()}]$)}
        \If{($r[i] \land$ $b[i] = ``"$)}
        \State return ``" \Comment{if bitwise AND is ever empty, entire string intersection is empty}
        \Else 
        \State ret $\leftarrow$ ret $+$ $r[i] \land b[i]$ \Comment{for each character, store bitwise AND}
        \EndIf
    \EndFor
    \State return ret
\EndProcedure
...
\Procedure{set\_intersect}{vector $R$, vector $B$, bool simp}
    \State Pad $R$ and $B$ to the same length
    \State ret $\leftarrow \{\}$ \Comment{initializing a vector of strings to store set intersection}
    \For{($r \in R$)}
        \For{($b \in B$)} \Comment{take bitwise AND of every combination of strings from $R$ and $B$}
            \State add bit\_wise\_and($r, b$) to ret 
            \Comment{store results in ret}
        \EndFor
    \EndFor
    \If{(simp is true)}
    \State return simplify(ret) \Comment{if user requests simplified output, run ret through simplify}
    \Else
    \State return ret
    \EndIf
\EndProcedure
...
\Procedure{join}{vector $R$, vector $B$, bool simp}
    \State ret $\leftarrow \{\}$ \Comment{initializing a vector of strings to store union}
    \For{($r \in R$)}
        \State add $r$ to ret \Comment{add all vectors in $R$ to ret}
    \EndFor
    \For{($b \in B$)}
        \State add $b$ to ret \Comment{add all vectors in $B$ to ret}
    \EndFor
    \If{(simp is true)}
    \State return simplify(ret) \Comment{if user requests simplified output, run ret through simplify}
    \Else
...
\Procedure{reg\_G}{vector reg\_alpha, int $a$, int $b$, int $n$, bool simp} 
    \State pre $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{a}$ \Comment{before the start of the interval, computation is arbitrary}
    \State comp $\leftarrow$ reg\_alpha \Comment{\parbox[t]{.7\linewidth}{initializing vector of strings for outputted regex with subformula regex}}
    \If{$a > b$} \Comment{empty interval, Global is trivially true}
        \State return $\{S^n\}$
    \EndIf
    \For{($1 \leq i \leq b-a$)} \Comment{computing $\bigwedge_{i=1}^{b-a} (S^n,)^i \text{reg}(\alpha)$}
        \State temp\_alpha $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{i}$ + reg\_alpha
        \State comp $\leftarrow$ set\_intersect(comp, temp\_alpha, simp)
    \EndFor
    \State return pre + comp \Comment{\parbox[t]{.6\linewidth}{prepending string pre to every string in comp; result is regex for Global defined in section \ref{regex}}}
\EndProcedure
...
\Procedure{reg\_F}{vector reg\_alpha, int $a$, int $b$, int $n$, bool simp} 
    \State pre $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{a}$ \Comment{before the start of the interval, computation is arbitrary}
    \State comp $\leftarrow$ reg\_alpha \Comment{\parbox[t]{.7\linewidth}{initializing vector of strings for outputted regex with subformula regex}}
    \If{$a > b$} \Comment{empty interval, Finally is trivially false}
        \State return $\{\}$
    \EndIf
    \For{($1 \leq i \leq b-a$) } \Comment{computing $\bigvee_{i=1}^{b-a} (S^n,)^i \text{reg}(\alpha)$}
        \State temp\_alpha $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{i}$ + reg\_alpha
        \State comp $\leftarrow$ join(comp, temp\_alpha, simp)
    \EndFor
    \State return pre + comp \Comment{\parbox[t]{.6\linewidth}{prepending string pre to every string in comp; result is regex for Finally defined in section \ref{regex}}}
\EndProcedure
...
\Procedure{reg\_U}{vector reg\_alpha, vector reg\_beta int $a$, int $b$, int $n$, bool simp} 
    \State comp $\leftarrow$ ((`$S$')$^{n}$ + `,')$^{a} + $ reg\_beta \Comment{\parbox[t]{.55\linewidth}{initializing vector of strings for outputted regex with regex for beta, prepended with $`S'$s up to start of interval}}
    \If{$a > b$}
        \State return $\{\}$ \Comment{empty interval, Until is trivially false}
    \EndIf
    \For{($a \leq i \leq b-1$) } \Comment{\parbox[t]{.65\linewidth}{computing $\bigvee_{i=a}^{b} \text{reg}\left(\mathcal{G}_{[a,i-1]}\alpha \land \mathcal{G}_{[i, i]} \beta\right)$, loop ends at $b-1$ because comp is initialized to $i=a$ case and loop is reindexed}}\\
        \State{\parbox[t]{.9\linewidth}{temp\_comp $\leftarrow$ set\_intersect(reg\_G(reg\_alpha, $a$, $i$, $n$), reg\_G(reg\_beta, $i+1$, $i+1$, $n$), $n$, simp)}}
...
\Procedure{reg\_R}{vector reg\_alpha, vector reg\_beta int $a$, int $b$, int $n$, bool simp} 
    \State comp $\leftarrow$ reg\_G(reg\_beta, $a$, $b$, $n$, simp)
    \Comment{\parbox[t]{.5\linewidth}{initializing vector of strings for output regex with regex for G[a:b]$\beta$}}
    \If{$a > b$}
        \State return $\{S^n\}$
        \Comment empty interval, Release is trivially true
    \EndIf
    \For{($a \leq i \leq b-1$) } \Comment{\parbox[t]{.65\linewidth}{computing $\text{reg}\left(\mathcal{G}_{[a,b]}\beta\right) \lor \bigvee_{i=a}^{b-1} \text{reg}\left(\mathcal{G}_{[a,i]}\beta \land \mathcal{G}_{[i, i]} \alpha\right)$, first run through loop includes $\text{reg}\left(\mathcal{G}_{[a,b]}\beta\right)$}}\\
...
\Procedure{reg}{string nnf, int $n$, bool sub, bool simp} 
    \If{(nnf is a prop. var. or $\scriptstyle{\sim}$prop. var)}
        \If{(sub)}
    ...
    \EndIf
    \\
    \If{(nnf is of the form unary\_temp\_conn interval alpha)}
    ...
    \EndIf
    \\
    \If{(nnf is of the form ``(Assoc\_prop\_conn[nnf\_array])")}
    \Comment{Rewrite nnf in non-array form}
        ...
        \For{(array entry alpha in nnf\_array)}
            \If{(alpha is first entry)}
...
\begin{algorithmic}
\algrestore{myalg}
    \If{(nnf is of the form alpha binary\_prop\_conn beta)}
        ...
        \EndIf\\
        \If{(binary\_prop\_conn $=$ ``=")}
            \Comment{Rewrite in terms of OR, AND, and NOT}
        ...
        \EndIf\\
        \If{(binary\_prop\_conn $=$ ``\textgreater")}
            \Comment{Rewrite in terms of OR, AND, and NOT}
        ...
        \EndIf
    \EndIf\\
   \If{(nnf is of the form alpha binary\_temp\_conn interval beta)}
        ...
        \EndIf
    \EndIf\\
    \State Throw exception \Comment{\parbox[t]{.7\linewidth}{If none of the previous statements held, there's an error with the formula nnf}}
    \State return \{\}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        ...
        \If{(depth = 0)}
            \State return $\{p, q\}$
            \Comment{\parbox[t]{.54\linewidth}{$p$ represents a propositional variable or its negation, $q$ represents a propositional constant}}
        \EndIf
        ...
        \Comment{\parbox[t]{.45\linewidth}{recursively generate all formulas of $1$ lower depth}}
        \For{(string $\alpha \in V$)}
            \State add ``G[a:b]" + $\alpha$ to template
            \State add ``F[a:b]" + $\alpha$ to template
            \Comment Add unary temporal operators to template
            \For{string $\beta \in V$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        ...
        \For{string $t \in $ tests}
            \For{char ch $\in t$}
                \If{ch $ = p$}
                \Comment{\parbox[t]{.52\linewidth}{replace $a$ with a random propositional variable or the negation of a random propositional variable}}
...
\begin{figure}[H]
    ...
    \node[emptynode](for_loop)[right=.75cm of body]{for r in R};
...
\begin{figure}[H]
    ...
    %Nodes
    \node[emptynode](inputs){strings r, b};
    ...
    \node[emptynode](emptytext)[rotate=35, left=.75cm of placeholder]{\small{r$[i] \land$ b$[i] = ``"$}};
...
\begin{figure}[H]
    ...
    %Nodes
    \node[emptynode](inputs){vectors R, B};
    \node[node](initialize)[below=of inputs]{Initialize vector Ret};
    \node[node](add_r)[below=of initialize]{Add r to Ret};
    \node[node](add_t)[below=of add_r]{Add b to Ret};
    \node[node](return)[below=of add_t]{return Ret};
    \node[emptynode](rtext)[right=of add_r]{for string r$\in$R};
    \node[emptynode](ttext)[left=of add_t]{for string b$\in$B};
...
\begin{figure}[H]
    ...
    %Nodes
    \node[emptynode](inputs){vectors R,B};
    \node[node](initialize)[below=of inputs]{Pad R and B, Initialize vector Ret=\{\}};
    ...
    \node[emptynode](inner_text)[right=1.25cm of inner_loop]{for b$\in$B};
