...
class MainWindow(QMainWindow):
    ...
    def run_west(self):
        ...
        if sys.platform == "win32":
            subprocess.run(f"..\\bin\\west.exe \"{self.formula}\"", 
                           shell=True)
        else: # linux or mac
            west_path = os.path.join(BIN_DIR, 'west')
            subprocess.run(f"{west_path} '{self.formula}'",
    ...
    def read_subformulas(self):
        ...
        self.subformulas = {}
        subformulas_file = os.path.join(OUTPUT_DIR, 'subformulas.txt')
        with open(subformulas_file, 'r') as f:
            lines = f.read()
...
class visualizeFormulaWindow(QWidget):
    ...
    def show_trace_help(self):
        ...
        label.setReadOnly(True)
        # read html from src/WEST/trace_help.html
        trace_help_file = os.path.join(SCRIPT_DIR, 'src', 'WEST', 'trace_help.html')
        with open(trace_help_file, 'r') as f:
