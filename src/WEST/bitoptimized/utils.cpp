...
/*
Adds commas every n characters. 
Input string s is of length a multiple of n.
*/
string add_commas(string s, int n) {
    string t = "";
    // iterate through s n characters at a time
...
vector<string> add_commas(vector<string> S, int n) {
    vector<string> T;
    for (int i = 0; i < S.size(); ++i) {
        T.push_back(add_commas(S[i], n));
    }
    return T;
}


// Follows this python code
// def trim_suffixes(L):
//     if len(L) <= 1:
//         return L
//     if all([w[-1] == 's' for w in L]) or all([w[-1] == ',' for w in L]):
//         return trim_suffixes([w[:-1] for w in L])
//     else:
//         return L
vector<string> trim_suffixes(vector<string> L) {
    if (L.size() <= 1) {
        return L;
    }
    bool all_s = true;
    ...
    if (all_s || all_comma) {
        vector<string> T;
        for (int i = 0; i < L.size(); ++i) {
            T.push_back(L[i].substr(0, L[i].length() - 1));
        }
        return trim_suffixes(T);
    }
    else {
        return L;
    }
